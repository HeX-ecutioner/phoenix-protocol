...
export function ScannerConsole() {
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await createScan(deviceType, [file]);
      
      // Simulate slightly longer processing time for cinematic effect
      setTimeout(() => {
        setStatus('success');
    ...
    <section id="scanner-console" className="w-full max-w-3xl mx-auto px-6 scroll-mt-32">
      <div className="glass-card p-8 shadow-2xl bg-neutral-900/60 backdrop-blur-xl border border-neutral-800/80 rounded-sm">
        ...
        <div className="mb-8">
          ...
          <div 
            className={`border-2 border-dashed rounded-sm p-12 text-center transition-all ${file ? 'border-brand-orange/50 bg-brand-orange/5' : 'border-neutral-800 hover:border-neutral-700 bg-neutral-950/50'} ${status !== 'idle' && status !== 'error' ? 'pointer-events-none opacity-50' : ''}`}
          ...
          >
            ...
            <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center justify-center">
              <FileUp className={`w-10 h-10 mb-4 ${file ? 'text-brand-orange' : 'text-neutral-600'}`} />
              <span className="text-white font-mono text-sm mb-2">
                {file ? file.name : "Drop configuration file here"}
              </span>
